    ...
    <div className="min-h-screen flex flex-col font-sans selection:bg-lg-red selection:text-white bg-white">
            ...
            {successMessage ? (
              ...
              <form onSubmit={handleRegister} className="space-y-5">
                ...
                <p className="text-[10px] text-slate-400 font-bold leading-relaxed pt-2">
                  By registering, you agree to our <button type="button" onClick={() => setShowTerms(true)} className="text-lg-red hover:underline decoration-2 underline-offset-2">Terms of Service</button> and <button type="button" onClick={() => setShowPrivacy(true)} className="text-lg-red hover:underline decoration-2 underline-offset-2">Privacy Policy</button>.
                </p>
      ...
      {showTerms && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[80vh] flex flex-col shadow-2xl overflow-hidden animate-slide-up">
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <h3 className="text-xl font-black text-slate-900 tracking-tight uppercase">Terms of Service</h3>
              <button onClick={() => setShowTerms(false)} className="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-full transition-colors">
                <X size={20} />
              </button>
            </div>
            <div className="p-8 overflow-y-auto flex-1 text-sm text-slate-600 leading-relaxed space-y-4">
              ...
              <h4 className="font-bold text-slate-900">2. User Registration</h4>
              <p>You are responsible for maintaining the confidentiality of your account credentials.</p>
              <h4 className="font-bold text-slate-900">3. Wholesale & Purchasing</h4>
              <p>All wholesale purchases are subject to minimum order quantities and verification of business status where applicable.</p>
              {/* Add more real terms here later */}
              <p className="text-xs text-slate-400 mt-8 uppercase tracking-wider font-bold">Last Updated: June 2026</p>
            </div>
            <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end">
              <button onClick={() => setShowTerms(false)} className="px-6 py-3 bg-lg-red text-white text-xs font-black uppercase tracking-widest rounded-xl hover:bg-red-700 transition-colors shadow-md">
                I Understand
              </button>
            </div>
          </div>
      ...
      {showPrivacy && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[80vh] flex flex-col shadow-2xl overflow-hidden animate-slide-up">
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <h3 className="text-xl font-black text-slate-900 tracking-tight uppercase">Privacy Policy</h3>
              <button onClick={() => setShowPrivacy(false)} className="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-full transition-colors">
                <X size={20} />
              </button>
            </div>
            <div className="p-8 overflow-y-auto flex-1 text-sm text-slate-600 leading-relaxed space-y-4">
              <h4 className="font-bold text-slate-900">1. Information We Collect</h4>
              <p>We collect information you provide directly to us, such as your name, email, phone number, and transaction details.</p>
              <h4 className="font-bold text-slate-900">2. How We Use Your Information</h4>
              <p>We use the information to fulfill orders, improve our services, and communicate with you about promotions and updates.</p>
              <h4 className="font-bold text-slate-900">3. Data Security</h4>
              <p>We implement industry-standard security measures to protect your personal information.</p>
              {/* Add more real privacy policy here later */}
              <p className="text-xs text-slate-400 mt-8 uppercase tracking-wider font-bold">Last Updated: June 2026</p>
            </div>
            <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end">
              <button onClick={() => setShowPrivacy(false)} className="px-6 py-3 bg-slate-900 text-white text-xs font-black uppercase tracking-widest rounded-xl hover:bg-slate-800 transition-colors shadow-md">
                Close
              </button>
            </div>
          </div>
